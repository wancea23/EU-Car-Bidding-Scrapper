...
## KNOWN BLOCKER — GitHub Actions is refused by CloudFront

The scheduled workflow does **not** currently work, and the cause is not this
code. On a runner the captcha is read correctly and answered correctly (grid
"choose all the curtains", model returned exactly the right five tiles), a
token is minted — and the very next request comes back:
...
Alcopa's WAF blocks GitHub Actions' Azure IP ranges outright, independently of
whether the CAPTCHA was solved. Solving it better cannot fix an IP block.

Everything runs correctly from a residential connection. Options, untested:
a small always-on machine on a home line, a VPS on a different ASN (may also
be blocked), or a residential proxy in front of the requests.
